...
const Navbar = ({ toggleTheme, theme }: { toggleTheme: () => void, theme: string }) => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 dark:bg-navy-950/95 backdrop-blur-sm shadow-md py-3' : 'bg-transparent py-4'}`}>
      <div className="container mx-auto px-4 lg:px-8 flex justify-between items-center">
        {/* Logo */}
        <a href="#" className="flex items-center gap-3 group relative z-[60]">
          <div className="w-10 h-10 bg-navy-900 dark:bg-white text-white dark:text-navy-900 flex items-center justify-center rounded-sm shadow-lg transition-transform group-hover:scale-105">
            <i className="fa-solid fa-scale-balanced text-lg"></i>
          </div>
          <div className="flex flex-col">
            <span className="font-serif font-bold text-lg md:text-xl leading-none text-navy-900 dark:text-white tracking-tight">
              Consultation House
            </span>
        ...
        <div className="hidden lg:flex items-center space-x-8">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="font-sans font-medium text-sm text-navy-800 dark:text-gray-300 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors uppercase tracking-wide"
            >
              {link.name}
            </a>
          ))}
          ...
          <button 
            onClick={toggleTheme}
            className="w-9 h-9 rounded-full flex items-center justify-center text-navy-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-navy-800 transition-colors"
          >
        ...
        <div className="flex items-center gap-4 lg:hidden relative z-[60]">
           <button 
            onClick={toggleTheme}
            className="w-8 h-8 flex items-center justify-center text-navy-900 dark:text-white"
          >
             <i className={`fa-solid ${theme === 'light' ? 'fa-moon' : 'fa-sun'}`}></i>
          </button>

          <button
            className="text-2xl text-navy-900 dark:text-white focus:outline-none"
      ...
      {/* Mobile Menu Overlay */}
      <div 
        className={`lg:hidden fixed inset-0 bg-white dark:bg-navy-950 z-[50] transition-all duration-300 ease-in-out flex flex-col pt-24 pb-8 px-6 ${isOpen ? 'opacity-100 visible translate-x-0' : 'opacity-0 invisible translate-x-full'}`}
      >
        <div className="flex flex-col h-full justify-between overflow-y-auto">
           <div className="flex flex-col space-y-2 mt-4">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="text-2xl font-serif font-bold text-navy-900 dark:text-white hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors py-4 border-b border-gray-100 dark:border-navy-800"
           ...
           <div className="mt-8 bg-gray-50 dark:bg-navy-900 p-6 rounded-xl">
             <p className="text-gray-500 dark:text-gray-400 text-xs uppercase tracking-widest font-bold mb-3">Quick Contact</p>
             <a 
                href={`tel:${CONTACT_INFO.phone}`} 
                className="text-2xl font-serif font-bold text-emerald-600 dark:text-emerald-400 block mb-2"
             >
                {CONTACT_INFO.phone}
             </a>
             <p className="text-sm text-navy-700 dark:text-gray-300 mb-4">{CONTACT_INFO.address}</p>
             <a href={`mailto:${CONTACT_INFO.email}`} className="text-sm font-medium text-navy-900 dark:text-white underline">{CONTACT_INFO.email}</a>
           </div>
        </div>
      </div>
    </nav>
...
const Hero = () => {
  return (
    <section id="home" className="relative min-h-screen flex items-center pt-28 pb-16 lg:pt-0 overflow-hidden bg-emerald-50/50 dark:bg-navy-950">
      
      {/* Background Elements */}
      <div className="absolute top-0 right-0 w-2/3 h-full bg-white dark:bg-navy-900 skew-x-[-12deg] translate-x-1/4 z-0 hidden lg:block"></div>
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
            ...
            <div className="w-full lg:w-1/2 text-center lg:text-left">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-400 text-xs font-bold uppercase tracking-wider mb-6 animate-fade-up">
                    <span className="w-2 h-2 rounded-full bg-emerald-600 animate-pulse"></span>
                    Serving Noida & Ghaziabad
                </div>
                
                <h1 className="text-4xl sm:text-5xl lg:text-6xl font-serif font-bold text-navy-900 dark:text-white leading-[1.15] mb-6 animate-fade-up delay-100">
                  Compliance <br/>
                  <span className="text-emerald-600 dark:text-emerald-500 relative">
                    Simplified.
                    <svg className="absolute w-full h-3 -bottom-1 left-0 text-emerald-200 dark:text-emerald-900 -z-10" viewBox="0 0 100 10" preserveAspectRatio="none">
                        <path d="M0 5 Q 50 10 100 5" stroke="currentColor" strokeWidth="8" fill="none" />
                    </svg>
                  </span>
                </h1>
                
                <h2 className="text-lg sm:text-xl font-sans text-navy-600 dark:text-gray-300 mb-6 animate-fade-up delay-200 font-medium">
                  फैक्ट्री और लेबर कानूनों का संपूर्ण समाधान
                </h2>
                
                <p className="text-base sm:text-lg text-gray-600 dark:text-gray-400 mb-8 leading-relaxed max-w-lg mx-auto lg:mx-0 animate-fade-up delay-300">
            ...
            <div className="w-full lg:w-1/2 flex justify-center lg:justify-end animate-fade-up delay-200">
                <div className="relative w-full max-w-sm lg:max-w-md">
                     <div className="bg-white dark:bg-navy-800 p-2 shadow-2xl rounded-sm rotate-3 border-4 border-white dark:border-navy-700">
                         <div className="bg-navy-900 h-64 lg:h-80 w-full flex flex-col items-center justify-center text-center p-6 relative overflow-hidden">
                             ...
                             <i className="fa-solid fa-scale-balanced text-6xl text-emerald-500 mb-4"></i>
...
const Services = () => {
  return (
    <section id="services" className="py-20 lg:py-24 bg-white dark:bg-navy-950">
      <div className="container mx-auto px-4 lg:px-8">
        
        <div className="text-center max-w-3xl mx-auto mb-16">
            <span className="text-emerald-600 dark:text-emerald-500 font-bold uppercase tracking-widest text-xs mb-3 block">What We Do</span>
            <h2 className="text-3xl lg:text-4xl font-serif font-bold text-navy-900 dark:text-white mb-6">Expertise Areas</h2>
          ...
          {SERVICES.map((service, index) => (
            <RevealOnScroll key={service.id} delay={index * 50}>
              <div className="group bg-gray-50 dark:bg-navy-900 p-6 lg:p-8 rounded-sm hover:bg-white dark:hover:bg-navy-800 border border-gray-100 dark:border-navy-800 hover:border-emerald-500 dark:hover:border-emerald-500 shadow-sm hover:shadow-elegant transition-all duration-300 h-full flex flex-col relative">
                ...
                <div className="w-12 h-12 bg-white dark:bg-navy-950 border border-gray-100 dark:border-navy-800 flex items-center justify-center text-emerald-600 dark:text-emerald-400 text-xl shadow-sm mb-6 group-hover:bg-emerald-600 group-hover:text-white transition-colors">
                    <i className={`fa-solid ${service.icon}`}></i>
                </div>
                
                <h3 className="text-lg lg:text-xl font-serif font-bold text-navy-900 dark:text-white mb-2 group-hover:text-emerald-700 dark:group-hover:text-emerald-400 transition-colors">
                  {service.title}
                </h3>
                <h4 className="text-sm font-medium text-emerald-600 dark:text-emerald-500 mb-4 font-sans">
                  {service.hindi}
                </h4>
                ...
                <div className="mt-auto pt-4 border-t border-gray-200 dark:border-navy-800">
                    <a href="#contact" className="inline-flex items-center text-xs font-bold uppercase tracking-widest text-navy-800 dark:text-gray-300 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
...
const About = () => {
  return (
    <section id="about" className="py-20 bg-navy-900 dark:bg-black text-white relative overflow-hidden">
      {/* Pattern */}
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        <div className="flex flex-col lg:flex-row gap-16 items-center">
          ...
          <div className="w-full lg:w-1/2 bg-white/5 backdrop-blur-sm p-8 border border-white/10 rounded-sm">
...
const Contact = () => {
  ...
  return (
    <section id="contact" className="py-20 lg:py-24 bg-emerald-50 dark:bg-navy-950">
      <div className="container mx-auto px-4 lg:px-8">
        <RevealOnScroll>
            <div className="max-w-5xl mx-auto bg-white dark:bg-navy-900 shadow-elegant rounded-sm overflow-hidden flex flex-col md:flex-row">
                
                <div className="w-full md:w-5/12 bg-navy-900 text-white p-10 flex flex-col justify-between relative overflow-hidden">
                     <div className="absolute -bottom-10 -right-10 text-navy-800 text-[150px] opacity-20">
                ...
                <div className="w-full md:w-7/12 p-8 lg:p-10">
                    <form className="space-y-6" onSubmit={handleSubmit}>
                        <div className="grid grid-cols-1 gap-6">
                             <div>
                                 <label className="block text-xs font-bold uppercase tracking-wide mb-2 text-navy-600 dark:text-gray-400">Name</label>
                                 <input type="text" required className="w-full bg-gray-50 dark:bg-navy-950 border border-gray-200 dark:border-navy-700 rounded-sm px-4 py-3 focus:outline-none focus:border-emerald-500 transition-all dark:text-white" placeholder="Full Name" />
                             </div>
                             <div>
                                 <label className="block text-xs font-bold uppercase tracking-wide mb-2 text-navy-600 dark:text-gray-400">Phone</label>
                                 <input type="tel" required className="w-full bg-gray-50 dark:bg-navy-950 border border-gray-200 dark:border-navy-700 rounded-sm px-4 py-3 focus:outline-none focus:border-emerald-500 transition-all dark:text-white" placeholder="Mobile Number" />
                             </div>
                        </div>
                        <div>
                             <label className="block text-xs font-bold uppercase tracking-wide mb-2 text-navy-600 dark:text-gray-400">Service</label>
                             <select className="w-full bg-gray-50 dark:bg-navy-950 border border-gray-200 dark:border-navy-700 rounded-sm px-4 py-3 focus:outline-none focus:border-emerald-500 transition-all dark:text-white">
                        ...
                        <div>
                             <label className="block text-xs font-bold uppercase tracking-wide mb-2 text-navy-600 dark:text-gray-400">Message</label>
                             <textarea rows={3} required className="w-full bg-gray-50 dark:bg-navy-950 border border-gray-200 dark:border-navy-700 rounded-sm px-4 py-3 focus:outline-none focus:border-emerald-500 transition-all dark:text-white resize-none"></textarea>
                        </div>
                        <div className="pt-2">
                            <button 
                              type="submit" 
...
const Footer = () => {
  return (
    <footer className="bg-white dark:bg-navy-950 border-t border-gray-100 dark:border-navy-900 pt-16 pb-8">
      <div className="container mx-auto px-4 lg:px-8">
        <div className="flex flex-col md:flex-row justify-between items-center text-center md:text-left gap-8 mb-12">
            
            <div className="flex items-center gap-3">
                 <div className="w-10 h-10 bg-emerald-600 flex items-center justify-center text-white text-lg rounded-sm">
                    <i className="fa-solid fa-scale-balanced"></i>
                </div>
                <div>
                  <h4 className="font-serif font-bold text-xl text-navy-900 dark:text-white">Consultation House</h4>
                  <p className="text-xs text-emerald-600 dark:text-emerald-500 uppercase tracking-widest">Industrial Experts</p>
                </div>
            </div>

            <div className="flex gap-8">
                <a href="#" className="w-10 h-10 flex items-center justify-center rounded-full bg-gray-100 dark:bg-navy-900 text-navy-600 dark:text-gray-400 hover:bg-emerald-600 hover:text-white transition-colors">
        ...
        <div className="border-t border-gray-100 dark:border-navy-900 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
...
const ChatWidget = () => {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[70] flex flex-col items-end font-sans">
      ...
      {!isOpen && (
        <div 
            className="mb-2 bg-navy-900 dark:bg-white text-white dark:text-navy-900 px-4 py-2 rounded-full text-xs font-bold shadow-xl cursor-pointer hover:scale-105 transition-transform border border-white/10 flex items-center gap-2" 
      ...
      {isOpen && (
        <div className="bg-white dark:bg-navy-900 w-[90vw] sm:w-[350px] h-[450px] max-h-[70vh] shadow-2xl rounded-lg border border-gray-200 dark:border-navy-700 flex flex-col mb-4 overflow-hidden animate-fade-up">
          {/* Header */}
          ...
          {/* Messages */}
          <div className="flex-1 bg-gray-50 dark:bg-navy-950 p-4 overflow-y-auto">
            <div className="space-y-4">
              ...
              {isTyping && (
                 <div className="flex justify-start">
                   <div className="px-3 py-2 bg-white dark:bg-navy-800 rounded-lg border border-gray-100 dark:border-navy-700 text-xs text-gray-500 flex items-center gap-1">
          ...
          {/* Input */}
          <form onSubmit={handleSend} className="p-3 bg-white dark:bg-navy-900 border-t border-gray-100 dark:border-navy-800 shrink-0">
            <div className="flex gap-2 relative">
              <input
                ...
                className="w-full bg-gray-50 dark:bg-navy-950 rounded-sm px-4 py-2 text-sm focus:outline-none focus:border-emerald-500 border border-transparent focus:border text-navy-900 dark:text-white transition-all"
              />
              <button 
                type="submit"
                disabled={!input.trim() || isTyping}
                className="w-10 h-9 rounded-sm bg-navy-900 dark:bg-emerald-600 text-white flex items-center justify-center hover:opacity-90 disabled:opacity-50 transition-all"
              >
      ...
      <button
        onClick={() => setIsOpen(!isOpen)}
        className={`w-14 h-14 rounded-full shadow-lg flex items-center justify-center text-xl transition-all duration-300 hover:scale-105 border-4 border-white dark:border-navy-800 ${isOpen ? 'bg-gray-800 text-white' : 'bg-emerald-600 text-white'}`}
      >
...
const App = () => {
  ...
  return (
    <div className="antialiased overflow-x-hidden min-h-screen bg-gray-50 text-navy-900 dark:bg-navy-950 dark:text-gray-100 transition-colors duration-300 font-sans">
      <Navbar {...themeProps} />
      <Hero />
      <Services />
      <About />
